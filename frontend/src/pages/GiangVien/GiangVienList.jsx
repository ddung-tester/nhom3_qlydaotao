...
// Giảng viên - pattern tương tự HocVien
export default function GiangVienList() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingId) {
                await axios.put(`${API_URL}/giangvien/${editingId}`, cleanedData);
            } else {
                await axios.post(`${API_URL}/giangvien`, cleanedData);
            }
            fetchData();
            resetForm();
    ...
    const handleDelete = async (row) => {
        if (window.confirm(`Xác nhận xóa giảng viên ${row.hoten}?`)) {
            try {
